...
static bool is_zygote(int pid){
    return (check_process(pid, nullptr, nullptr, "/system/bin/app_process")
            ...
            && (check_process(pid, "zygote", "u:r:zygote:s0", nullptr)  
...
static void check_zygote(){
    crawl_procfs([](int pid) -> bool {
        ...
        if (check_process(pid, "system_server", "u:r:system_server:s0", nullptr)
...
static void inotify_event2(int) {
    // stop all zygote if possible
    POLL_EVENT
    for (auto it = zygote_map.begin(); it != zygote_map.end(); it++) {
        kill(it->first, SIGSTOP);
    }
    PROCESS_EVENT
    for (auto it = zygote_map.begin(); it != zygote_map.end(); it++) {
